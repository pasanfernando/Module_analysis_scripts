...
'''
Use this script for mouse thresholds
...
if input1 == 'y':
    ...
    # if yes please check the name of the filehindlimbgenelist_including_parts.txt
...
othergenelist = open('pelgenelist_including_parts.txt', 'r')
...
thresholddic ={}
prefile = open('uberon_0002103precision_hash.txt', 'r')
for line in prefile:
    if line != '\n':
        a=line.strip().split('\t')
...
itrange= np.linspace(0,50,100)
ftrange = np.linspace(50,max(thresholddic.keys()),100)
...
for threshold in trange:
    ...
    for gene in onlypredicted:
        ...
        elif gene in mousedic:
            if mousedic[gene] in otherlist:
                commongenes.append(gene)
